 ...
 * @author esdra
 */
public class RepositorioAluguel implements RepositorioGenerico  {
    ...
    public Object recuperar(Object cliente) {
        try {
            return (Aluguel) DaoManagerHiber.getInstance().recover("from Relatorio where cliente" + cliente).get(0);
    ...
    @Override
    public List<Aluguel> recuperarTodos() {
        return DaoManagerHiber.getInstance().recover("from Aluguel");
    }
}
